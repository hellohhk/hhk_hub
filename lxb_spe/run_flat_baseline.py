...
from spe_full import BBHEvaluator, DeepSeekKernel, load_deepseek_config
from spe_full.embedding import HashingNgramEmbedder
from spe_full.flat import FlatGenome, FlatOptimizer, FlatOptimizerConfig
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="logs_flat_baseline")
    p.add_argument("--init_prompt", type=str, default="You are a helpful assistant. Solve the problem.")
    return p.parse_args()
...
def main():
    ...
    optimizer_cfg = FlatOptimizerConfig(
        budget=args.budget,
        mu=args.mu,
        lambd=args.lambda_,
        gens=args.gens,
    ...
    optimizer = FlatOptimizer(kernel=kernel, cfg=optimizer_cfg, embedder=embedder)

    init_pop = [FlatGenome(text=args.init_prompt, uid=f"flat_init_{i}", operator="init") for i in range(args.mu)]
    log_fn({"phase": "run_start", "bbh_cache_dir": args.bbh_cache_dir, "bbh_tasks": tasks, "bbh_n_shot": args.bbh_n_shot, "cfg": _to_jsonable(optimizer_cfg)})
