...
from spe_full import BBHEvaluator, DeepSeekKernel, SPEOptimizer, SPEOptimizerConfig, StructuredGenome, load_deepseek_config
...
def run_one(task_group_name: str, args) -> dict:
    ...
    log_path = Path(args.log_dir) / f"{task_group_name}_{run_id}.jsonl"
    ...
    for i in range(args.mu):
        ...
        init_pop.append(StructuredGenome(loci=loci, uid=f"{task_group_name}_init_{i}", operator="init"))
...
def main():
    args = parse_args()
    run_name = "bbh"
    if args.bbh_tasks.strip():
        run_name = "bbh_" + "_".join([t.strip() for t in args.bbh_tasks.split(",") if t.strip()])
    result = run_one(run_name, args)
